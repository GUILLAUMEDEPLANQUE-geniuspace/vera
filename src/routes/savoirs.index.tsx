...
export const Route = createFileRoute("/savoirs/")({
  ...
  head: () => ({
    meta: [
      { title: "Fiches Vera — hub de connaissance pour l’emploi et la formation" },
      {
        name: "description",
        content:
          "Fiches métier : marché, robotique, droit, compta, création d’entreprise. Liées aux offres. Un module avant de postuler si le geste manque.",
      },
      { name: "robots", content: "index,follow" },
      { property: "og:title", content: "Fiches Vera — le geste avant le titre" },
    ],
    links: [{ rel: "canonical", href: `${BRAND_HOST}/savoirs` }],
    scripts: [
      ldScript(
        itemListJsonLd("Fiches Vera", [
...
function SavoirsIndex() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6">
      ...
      <p className="mt-3 text-xs tracking-wide text-primary uppercase">Le geste avant le titre</p>
      <h1 className="mt-2 max-w-3xl font-serif text-4xl sm:text-6xl">
        Le hub où le métier s’écrit — et mène à l’offre.
      </h1>
      <p className="mt-4 max-w-2xl text-lg text-muted">
        Pas un forum. Pas un LMS. Des fiches tenues par les entreprises et les candidats : marché, droit, compta,
        robotique, terrain. Chaque fiche ouvre des offres. Si le geste manque, vous{" "}
        <Term k="preform">suivez un module</Term> ici, puis vous tenez l’
      ...
      {featured && (
        <article className="mt-16 overflow-hidden rounded-2xl border border-border bg-surface">
          ...
          <div className="p-6 sm:p-8">
            ...
            <p className="mt-3 text-xs text-subtle">
              {featured.authorName} · {featured.authorRole === "house" ? "entreprise" : featured.authorRole === "candidate" ? "candidat" : "Vera"}
            </p>
          </div>
        </article>
      )}

      <h2 className="mt-16 font-serif text-3xl">Fil — entreprises et candidats</h2>
        ...
        {rest.map((a) => (
          <li key={a.slug} className="rounded-2xl border border-border bg-surface p-5">
            ...
            <p className="mt-2 text-xs text-subtle">
              {a.authorName} · {a.authorRole === "house" ? "entreprise" : a.authorRole === "candidate" ? "candidat" : "Vera"}
            </p>
          </li>
        ))}
      </ul>
